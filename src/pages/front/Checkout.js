...
function Checkout() {
    ...
    const getCart = async () => {
        ...
        setCart(data.docs.map((doc) => ({...doc.data(), isSelected: false, id: doc.id })).filter(cartItem => selected.includes(cartItem.id)))
        const userCollectionRef = collection(db, "users")
        const dataUsers = await getDocs(userCollectionRef)
        const validUser = dataUsers.docs?.map((doc) => ({...doc.data(), id: doc.id })).find(user => user.Email === sessionStorage.getItem('email'))
        setUser(validUser)
    }
    getCart();
    }, []);
        ...
        <div style={{minHeight: 'calc(100vh) - 100px', marginTop: '100px'}}>
            <Header />
                <Paper sx={{width: '70%', margin: 'auto'}} elevation={2} >
                    <div>
                    <Typography variant='Title'>Delivery Address: <LocationOnIcon /> </Typography>
                    </div>
                    <div style={{display: 'flex', justifyContent: 'space-between'}}>
                        <div>
                            <Typography variant='h6'>Name: {user.Name}</Typography>
                            <Typography variant='h6'>Phone Number: {user.PhoneNum}</Typography>
                        </div>
                            ...
                            <>
                                <Paper className="cart-list" elevation={2}>
                                    ...
                                    <div className="box-2">
                                        ...
                                        <div className="box"><label id="totalprice"> Total Price: ₱ {(cart.Price * cart.Quantity).toLocaleString()}</label></div>
                                    </div>
                                </Paper>
                            </>
                        );
