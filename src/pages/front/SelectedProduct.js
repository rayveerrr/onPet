...
const SelectedProduct = () => {
    ...
    useEffect(() => {
        const getProduct = async () => {
            ...
            const prod = data.docs.map((doc) => ({...doc.data(), id: doc.id })).find(product => product.id === params.id)
            setProduct(prod)
            console.log(prod)
            console.log(product)
        }
    ...
    const submitAddToCart = async () => {
        ...
        try {
            await addDoc(cartCollectionRef, { ProdID: params.id, Email: sessionStorage.getItem('email'), Image: product.Image || '', Price: product.Price , ProdName: product.ProdName , Quantity: quantityCount})
                    ...
                    <div className="availability-container">
                        ...
                        <p> 
                            <Stack direction="row" alignItems='center'> Quantity:
                                <IconButton aria-label="minus" onClick={decrement} disabled={quantityCount == 1} >
